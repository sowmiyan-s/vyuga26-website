...
interface UpdateEventSelectorProps {
    selectedEvents: string[];
    onChange: (events: string[]) => void;
    error?: string;
}

const UpdateEventSelector = ({
    selectedEvents,
    onChange,
    error
}: UpdateEventSelectorProps) => {
    ...
    const handleToggle = (eventId: string) => {
        ...
        // If event is already selected, allow deselection
        if (selectedEvents.includes(eventId)) {
            onChange(selectedEvents.filter(id => id !== eventId));
            return;
        }

        // Check if we already have an event from this category
        const existingEventFromSameCategory = selectedEvents.find(id => {
            const selectedEvent = events.find(e => e.id === id);
            return selectedEvent?.category === event.category;
        });

        if (existingEventFromSameCategory) {
            toast.error(`You already selected a ${event.category} event. Please deselect it first.`);
            return;
        }
    ...
    const getTechnicalCount = () => {
        return selectedEvents.filter(id => {
            const event = events.find(e => e.id === id);
            return event?.category === "technical";
        }).length;
    };

    const getNonTechnicalCount = () => {
        return selectedEvents.filter(id => {
            const event = events.find(e => e.id === id);
    ...
    const EventCard = ({ event }: { event: typeof events[0] }) => {
        ...
        const isClosed = event.isRegistrationClosed || settings.registration_closed_events?.[event.id];

        // Check if this category already has a selected event
        const categoryHasSelection = selectedEvents.some(id => {
            const selectedEvent = events.find(e => e.id === id);
            return selectedEvent?.category === event.category && selectedEvent.id !== event.id;
        });

        const isDisabled = !isSelected && (isClosed || categoryHasSelection);
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div>
                        <h3 className="text-white font-bold text-lg flex items-center gap-2">
                            Select Events
                            <span className="text-xs font-medium px-2 py-0.5 rounded-full border bg-white/5 text-gray-400 border-white/10">
                                {selectedEvents.length}/2
                            </span>
                        </h3>
                        <div className="flex gap-3 mt-2 text-xs">
                            <span className={`px-2 py-1 rounded-md ${getTechnicalCount() === 1 ? 'bg-cyan-500/20 text-cyan-400 border border-cyan-500/30' : 'bg-white/5 text-gray-400'}`}>
                                Technical: {getTechnicalCount()}/1
                            </span>
                            <span className={`px-2 py-1 rounded-md ${getNonTechnicalCount() === 1 ? 'bg-fuchsia-500/20 text-fuchsia-400 border border-fuchsia-500/30' : 'bg-white/5 text-gray-400'}`}>
                                Non-Tech: {getNonTechnicalCount()}/1
                            </span>
                        </div>
                    </div>
...
export default UpdateEventSelector;
